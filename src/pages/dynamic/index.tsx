...
class Dynamic extends Component<PropsWithChildren> {
  config: Config = {
  ...
  initCoverData = () => {
    ...
    // console.log("coverList", sceneConfig)
    this.setState({
      coverList: coverList
    })
  ...
  handleChangeStyle = (data) => {
    // console.log('handlechangestyle', data)
    const {foreground} = this.state
    this.setState({
      foreground: {
        ...foreground,
  ...
  handleChooseScene = (scene) => {
    // console.log('handlechoosescene', scene)
    const {currentScene} = this.state
